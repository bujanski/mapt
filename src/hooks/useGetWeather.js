...
function extractHourWeather(data, hour) {
    ...
    for (const arrayName in data) {
        if (data.hasOwnProperty(arrayName)) {
            ...
            // Check if the array is long enough and has an element at the specified hour
            // index
...
async function getWeather(lat, long, date) {
    ...
    if (timeSplit > histTimeSplit) {
        const historicalWeather = await axios.get(`https://archive-api.open-meteo.com/v1/archive?latitude=${lat}&longitude=${long}&start_date=${trimmedDate}&end_date=${trimmedDate}&hourly=temperature_2m,precipitation,surface_pressure,cloud_cover,wind_speed_10m,wind_direction_10m&temperature_unit=fahrenheit&wind_speed_unit=mph&precipitation_unit=inch`);
        let hourPosition = historicalWeather
            .data
            .hourly
            .time
        ...
        const recentWeather = await axios.get(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${long}&hourly=temperature_2m,precipitation,surface_pressure,cloud_cover,wind_speed_10m,wind_direction_10m&temperature_unit=fahrenheit&wind_speed_unit=mph&precipitation_unit=inch&past_days=5&forecast_days=1`);
        let hourPosition = recentWeather
            .data
            .hourly
            .time
            .findIndex((element) => element === date);
